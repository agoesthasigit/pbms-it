-- 2026-08-27
-- Pemeriksaan Data B2 ("Nomor invoice format tidak standar") salah menandai
-- invoice brand Cetak Ide (prefix CTK/YYYY/MM/NNN) sebagai tidak standar, karena
-- regex-nya hanya mengizinkan prefix INV. Sejak fitur 2 brand (2026-08-21) prefix
-- CTK adalah format SAH → regex diperlebar menerima INV maupun CTK.
--
-- HANYA baris B2 yang berubah. Fungsi ini murni pelaporan (read-only) — tidak
-- menyentuh alur invoice, penomoran, maupun jatuh tempo. CREATE OR REPLACE,
-- tanda tangan sama; sisa cek A/C/D/E/F identik dengan versi 20260822.
...
begin
  ...
  -- B2: terima prefix INV (Athaya) maupun CTK (Cetak Ide)
  select count(*) into b2 from monthly_invoices
    where user_id=u and invoice_no !~ '^(INV|CTK)/\d{4}/\d{2}/\d+$';
  ...
  -- Grup F — Portal Distributor
  select count(*) into f1 from distributor_orders o
    where o.owner_user_id=u and o.status='accepted'
      and (o.purchase_id is null
           or not exists (select 1 from purchases pu where pu.id=o.purchase_id and pu.user_id=u));
  ...
  return jsonb_build_array(
    ...
    jsonb_build_object('code','F1','label','Pengajuan distributor diterima tanpa pembelian valid','count',f1)
  );
end $function$;
